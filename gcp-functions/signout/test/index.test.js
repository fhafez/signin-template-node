...
describe('functions_http_get_method', () => {
  it('http:signout: should handle GET with no patientID', (done) => {
    ...
    signout(req, res, () => {
      ...
      assert.deepStrictEqual(res.send.firstCall.args, ['{error: \"Must provide patientID\"}']);
      done();
    });

  });

  it('http:signout: should handle GET with a patientID but no signoutTime', (done) => {
    const name = uuid.v4();
    const req = {
      method: 'GET',
      query: {
  ...
  it('http:signout: should handle GET with a patientID and a signoutTime', (done) => {
